...
func (h *ContainerHandler) GetContainerLogs(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "No such container") || strings.Contains(err.Error(), "not found") {
			telemetry.WithRequestID(r.Context()).Warn("container not found for logs", "container_id", containerID, "error", err)
			http.Error(w, "Container not found", http.StatusNotFound)
			return
		}
		telemetry.WithRequestID(r.Context()).Error("failed to retrieve container logs", "container_id", containerID, "error", err)
...
func (h *ContainerHandler) SearchLogs(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		telemetry.WithRequestID(r.Context()).Error("log search failed", "query", query, "container", containerName, "error", err)
